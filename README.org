...
A NixOS-based fossil-server cluster for self-hosting personal projects. Three hosts (canonical + 2 secondaries) replicate via fossil's built-in sync. Native TLS, no reverse proxy. The flake lives in a private GitHub repository, with agenix-encrypted secrets.
...
| =fossil1.exidia.com= | secondary-1 | Hetzner DC2  |
| =fossil2.exidia.com= | secondary-2 | DigitalOcean |
