...
The =security.acme= module reads this as an env file. Format (one line, no quotes around the value, no trailing newline if your editor lets you control that):
...
agenix uses =$EDITOR= for the temp file — set it to whatever you prefer (=export EDITOR=vim= etc.) before running the command.
