...
Rotate one of the agenix-managed secrets (fossil-sync / tailscale auth keys / tmk-password / healthchecks URLs).

* Prerequisites

- Access to the agenix admin keypair.
- Network access to the GitHub repo (for push) and to every host that consumes the secret.
...
agenix -e <name>.age -i ~/.ssh/id_ed25519_ext
