...
If the format is wrong (e.g. raw token without the =VAR== prefix), re-encrypt via [[file:rotate-secrets.org][rotate-secrets]] → cloudflare-dns and ensure the file is an env file.
